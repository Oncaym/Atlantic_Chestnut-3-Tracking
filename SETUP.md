# Tracker Cloud Sync — Setup Guide

End-to-end: from zero to "team is editing the same dashboard live."
Applies to **any** tracker (AC3, CP2, or a new one generated by Project Sync).
Each project is independent — its own Firebase project + its own deployment.
**Time: ~20 minutes** for the core (Firebase + deploy); the chat feature needs
one extra key — see **Step 4**. No server needed; free on typical usage.
...
Files involved:
- **`index.html`** — the cloud-enabled dashboard
- **`cloud-sync.js`** — sync engine (core, don't edit)
...
- **`api/parse.js`** — chat updater endpoint (needs `ANTHROPIC_API_KEY`, see Step 4)
...
2. Name it after the building (e.g. `ac4-tracker`). Disable Google Analytics — you don't need it.
...
1. Drop the **whole folder** (`index.html`, `cloud-sync.js`, `firebase-config.js`, `api/`) into a SharePoint document library
...
## Step 4 — Environment variables & secrets (per deployment)

Firebase (Steps 1–3) covers login + live data. Two extra features are wired
through **Vercel environment variables** / **GitHub secrets** — set them on the
project's own deployment. **After adding any of these, redeploy** — env changes
don't apply to an existing deployment.

### a) Chat updater — `ANTHROPIC_API_KEY` (Vercel)
The natural-language chat page (`/chat`, via `api/parse.js`) needs an Anthropic key.
- Vercel → your project → **Settings → Environment Variables**
- Add `ANTHROPIC_API_KEY` = your key from https://console.anthropic.com → API Keys → **Redeploy**
- **Never put this key in a file in the repo** — it is server-side only, and the repo/Hub may be public.
- Billing: the key bills to *its* Anthropic account. Reusing one key across trackers
  shares that credit pool; issue separate keys (with per-key spend limits) to isolate or cap a project.
- Symptom if missing: chat shows `500 … ANTHROPIC_API_KEY not set in Vercel env vars`.

### b) Nightly backup — GitHub repo secrets (optional, recommended)
The Action `.github/workflows/backup.yml` commits a daily snapshot of your Firebase
data into `backups/`. In the project's GitHub repo → **Settings → Secrets and
variables → Actions**, add:
- `FIREBASE_SERVICE_ACCOUNT` = the full JSON from Firebase Console → Project settings →
...
Then run the Action once manually (repo → Actions tab) to verify.

### c) Contribute-back endpoint — `GITHUB_HUB_TOKEN` (Hub only)
Only the **Hub** deployment (the one hosting `api/contribute.js`) needs this — regular
tracker deployments do **not**. Setup lives in `HUB-SETUP.md`.

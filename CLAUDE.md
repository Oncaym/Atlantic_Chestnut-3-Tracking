...
> **接班必读顺序：本文件 → SYNC.md → HANDOFF.md（待办任务的执行规格和 Q&A 都在那）**
...
> **2026-07-06 架构变更：core/config 分离。** `app.js`/`app-log.js`/`cloud-sync.js`/`chat.html`/`api/parse.js`
> 现在是 **core 文件**，与 CP2 tracker 逐字节一致，可跨项目直接同步（规则见 `SYNC.md`，功能台账见 `FEATURES.md`）。
> 项目专属数据全部在 **`project-config.js`**（SEED、键名、门规则、项目名）。

| 文件 | 作用 | 能不能改 |
|---|---|---|
| `project-config.js` | **本项目全部专属数据**：SEED、storageKey、门 ID 规则、i18n 项目名 | scope 变更改这里 |
| `index.html` | 主看板（混合件：core 结构 + 项目 scope 卡片/标题/底图） | UI 改动在这里，跨项目只按功能块移植 |
| `app.js` | 全部逻辑（core，读 window.PROJECT） | 改逻辑=改 core，改完要同步到 CP2 |
| `state.json` | 种子数据的 JSON 版（units / log / positions） | 与 project-config.js 保持同步 |
| `cloud-sync.js` | Firebase 同步引擎（core） | 品牌已参数化，**不要改** |
...
3. **Scope 变更要改两处：** `project-config.js` 的 seedUnits + `state.json`，保持一致（app.js 不再含 SEED）。

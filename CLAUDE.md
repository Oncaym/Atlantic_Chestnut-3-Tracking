...
**Log（每日日志）：** `{ date, category, content, planned? }`，
category ∈ framing / glass / louver / issue。
...
- [ ] `firebase-config.js` 还是占位符 → 按 README 建新 Firebase 项目（**不要复用 CP2 的**）
- [ ] `gf-plan.png` 是占位图 → 换成真实 AC3 底图后用 Place mode 摆 SF 标记
- [ ] `warehouse.html` 尚未配 AC3 数据
- [ ] 立面视图目前只有 SF01
- [ ] chat 功能需要在 Vercel 设 `ANTHROPIC_API_KEY`
